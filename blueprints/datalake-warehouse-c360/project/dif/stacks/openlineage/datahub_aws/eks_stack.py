...
from aws_cdk import App, NestedStack, Environment, CfnOutput, RemovalPolicy
from aws_cdk import (
    aws_ec2 as ec2,
    aws_eks as eks,
    aws_iam as iam,
...
class EKSClusterStack(NestedStack):

    def __init__(self, scope: Construct, id: str,resource_prefix:str, vpc:ec2.Vpc=None ,**kwargs) -> None:
    ...
    def _create_iam_role(self) -> None:
            if (self.node.try_get_context("create_new_cluster_admin_role") == "True" or self.node.try_get_context(
                                                                "existing_admin_role_arn") == None):
    ...
    def _create_vpc(self) -> None:
            ...
            elif(self.node.try_get_context("existing_vpc_name")):
                if(self.eks_vpc == None):
        ...
        # Create an EKS Cluster
            self.eks_cluster = eks.Cluster(
                ...
                version=eks.KubernetesVersion.of(
                    self.node.try_get_context("eks_version") or "1.28"),
                default_capacity=0,
