...
with DAG(
    dag_id=f"{PROJECT_NAME_RAW}_emr_dag",
...
#         )
    ...
    dbt_tg = DbtTaskGroup(
        project_config=ProjectConfig(
            dbt_project_path=Path("/usr/local/airflow/dags/dbt/datalake")
        ...
        profile_config=ProfileConfig(
            ...
            profile_mapping=SparkThriftProfileMapping(
                conn_id = f'{PROJECT_NAME_RAW}_emr_connection',
                profile_args = { 'schema':PROJECT_NAME_RAW+"gold"}, 
            )

        ),
        render_config = RenderConfig(
            env_vars = {"RAW_BUCKET": RAW_BUCKET}
        ),
    ...
    job_terminate = EmrTerminateJobFlowOperator(
    ...
    generate_dbt_docs_aws = DbtDocsS3Operator(
        ...
        project_dir=Path("/usr/local/airflow/dags/dbt/datalake"),
        profile_config=ProfileConfig(
            profile_name="default",
            target_name="dev",
            profile_mapping=SparkThriftProfileMapping(
        ...
        folder_dir=f"{PROJECT_NAME_RAW}-lake",
        vars= {
            "ts": TIMESTAMP,
            "raw_bucket":RAW_BUCKET,
            "year":YEAR,
    ...
    setup_emr_autoscaling_transient_cluster >> e1 >>   dbt_tg >> e2  >> generate_dbt_docs_aws >> job_terminate
